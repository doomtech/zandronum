...
HRESULT CLCDGfx::Initialize(int nWidth, int nHeight)
{
    ...
    if(NULL == m_hDC)
    {
        Printf(_T("CLCDGfx::Initialize(): failed to create compatible DC.\n"));
        Shutdown();
        return E_FAIL;
    }
    ...
    {
        Printf(_T("CLCDGfx::Initialize(): failed to allocate bitmap info.\n"));
        Shutdown();
    ...
    if(NULL == m_hBitmap)
    {
        Printf(_T("CLCDGfx::Initialize(): failed to create bitmap.\n"));
        Shutdown();
        return E_FAIL;
    }
    ...
    {
        Printf(_T("CLCDGfx::Initialize(): failed to allocate the lcd screen structure.\n"));
        Shutdown();
        return E_OUTOFMEMORY;
    }
