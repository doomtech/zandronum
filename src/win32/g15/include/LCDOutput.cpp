...
HRESULT CLCDOutput::Draw()
{
    ...
    if (m_pActiveScreen)
    {
		...
		dwPriorityToUse = LGLCD_ASYNC_UPDATE(LGLCD_PRIORITY_ALERT);
    }
    else
    {
...
{
	if(dwTimestamp == INFINITE)
		m_nPriority = LGLCD_PRIORITY_ALERT;
	else
		m_nPriority = LGLCD_PRIORITY_IDLE_NO_SHOW;
...
void CLCDOutput::ReadButtons()
{
    if(IsOpened())
    {
        ...
        if (ERROR_SUCCESS != res)
        {
            Printf(_T("lgLcdReadSoftButtons failed: unplug?\n"));
            HandleErrorFromAPI(res);
        }
...
void CLCDOutput::HandleButtonState(DWORD dwButtonState, DWORD dwButton)
{
    ...
    {
        //Printf(_T("Button 0x%x released\n"), dwButton);
        OnLCDButtonUp(dwButton);
    }
    if ( !(m_dwButtonState & dwButton) && (dwButtonState & dwButton) )
    {
        //Printf(_T("Button 0x%x pressed\n"), dwButton);
...
void CLCDOutput::HandleErrorFromAPI(DWORD dwRes)
{
    ...
    case ERROR_DEVICE_NOT_CONNECTED:
        Printf(_T("lgLcdAPI returned with ERROR_DEVICE_NOT_CONNECTED, closing device\n"));
        OnClosingDevice(m_hDevice);
        break;
    default:
        Printf(_T("lgLcdAPI returned with other error (0x%08x) closing device and connection\n"));
        OnClosingDevice(m_hDevice);
...
void CLCDOutput::OnClosingDevice(int hDevice)
{
    ...
    // Printf(_T("CLCDOutput::OnClosingDevice\n"));
    if (IsOpened())
    {
        lgLcdClose(m_hDevice);
        m_hDevice = LGLCD_INVALID_DEVICE;
...
void CLCDOutput::OnDisconnecting(int hConnection)
{
    ...
    // Printf(_T("CLCDOutput::OnDisconnecting\n"));
